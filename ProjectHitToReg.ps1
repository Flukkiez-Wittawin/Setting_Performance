...
function Start-FiveM-Optimization {
  ...
  if ($Mode -ne "Mode13" -and $Mode -ne "Mode14") {
    ...
    # Mode 18 (Insane Pro) elite custom speed
    if ($Mode -eq "Mode18") {
      $repeatDelay = 110
      $repeatRate = 8
    }
...
"@ -EA SilentlyContinue
  ...
  if (Test-Path $fivemAppData) {
    ...
    $sleepSecs = if ($Mode -eq "Mode9" -or $Mode -eq "Mode10" -or $Mode -eq "Mode17" -or $Mode -eq "Mode18") { 1 } else { 3 }
...
for ($i = 1; $i -le 18; $i++) { $allCards += "Mode${i}Card" }
