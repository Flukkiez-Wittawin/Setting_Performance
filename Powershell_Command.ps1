...
====================================================================
          FIVEM & WINDOWS MASTER PVP OPTIMIZATION SCRIPT
                    VERSION: 2026_ULTIMATE_EDITION (ALL-IN-ONE)
                    ====================================================================
                    #>
                    ...
                    if (!([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
                        Write-Host "กำลังขอสิทธิ์ Administrator..." -ForegroundColor Yellow
                            Start-Process powershell.exe "-NoProfile -ExecutionPolicy Bypass -File `"$PSCommandPath`"" -Verb RunAs
                                ...
                                Write-Host "[1/9] ตั้งค่า FiveM เป็น High Priority ถาวร..." -ForegroundColor Cyan
                                ...
                                Write-Host "[2/9] ปลดล็อคพลังพลังงาน CPU (Ultimate Performance)..." -ForegroundColor Cyan
                                ...
                                if ($ultimatePlan) {
                                    $planGuid = $ultimatePlan.Line.Split()[3]
                                        powercfg -setactive $planGuid
                                            ...
                                            Write-Host "[3/9] เปิดใช้งาน HAGS และลดความหน่วงโครงสร้าง Windows..." -ForegroundColor Cyan
                                            ...
                                            Write-Host "[4/9] ปรับแต่ง Latency คีย์บอร์ด (Keyboard Response Speed)..." -ForegroundColor Cyan
                                            ...
                                            Write-Host "[5/9] ตั้งค่าเมาส์ตรงตามมือ (Disable Mouse Acceleration)..." -ForegroundColor Cyan
                                            ...
                                            Write-Host "[6/9] ปรับแต่งระเบียบการส่งข้อมูล Network (TCP No Delay / Ack Frequency)..." -ForegroundColor Cyan
                                            ...
                                            foreach ($i in $interfaces) {
                                                Set-ItemProperty -Path $i.PSPath -Name "TcpAckFrequency" -Value 1 -Type DWord -ErrorAction SilentlyContinue | Out-Null
                                                    ...
                                                    Write-Host "[7/9] บังคับสิทธิ์ Multimedia ให้ความสำคัญกับกราฟิกเกมสูงสุด..." -ForegroundColor Cyan
                                                        ...
                                                        Set-ItemProperty -Path $gpaPath -Name "GPU Priority" -Value 8 -Type DWord -Force | Out-Null
                                                            Set-ItemProperty -Path $gpaPath -Name "Priority" -Value 6 -Type DWord -Force | Out-Null
                                                                Set-ItemProperty -Path $gpaPath -Name "Scheduling Category" -Value "High" -Force | Out-Null
                                                                    ...
                                                                    Write-Host "[8/9] จัดการหน่วยความจำสำรอง (Virtual Memory) บนไดรฟ์ C:..." -ForegroundColor Cyan
                                                                        ...
                                                                        $PageFile.InitialSize = 16384
                                                                            $PageFile.MaximumSize = 32768
                                                                                Set-CimInstance -CimInstance $PageFile | Out-Null
                                                                                    ...
                                                                                    Write-Host "[9/9] ทำความสะอาดไฟล์ขยะระบบและล้าง Cache FiveM..." -ForegroundColor Cyan
